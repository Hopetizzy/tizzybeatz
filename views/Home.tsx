...
const Home: React.FC<HomeProps> = ({ products, isLoading, onPlayTrack, onAddToCart, onDownload, currentTrackId, isPlaying, onCreateCollab, searchQuery }) => {
  const [activeCategory, setActiveCategory] = useState('all');
  const [showCollabModal, setShowCollabModal] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);
  const [toast, setToast] = useState<{ message: string, visible: boolean }>({ message: '', visible: false });
  const [demoUrl, setDemoUrl] = useState('');
  const [activeLicenseProduct, setActiveLicenseProduct] = useState<Product | null>(null);
  ...
  const handleAddClick = (product: Product) => {
    if (product.isFree || product.type !== 'beat') {
       onAddToCart(product);
       setToast({ message: `"${product.title}" added to your cart`, visible: true });
    } else {
       setActiveLicenseProduct(product);
    }
  };

  const handleSelectLicense = (product: Product, tier: any, price: number) => {
    onAddToCart({ ...product, agreedPrice: price, licenseType: tier } as any);
    setActiveLicenseProduct(null);
    setToast({ message: `"${product.title}" (${tier.toUpperCase()}) added to your cart`, visible: true });
  };
  ...
  return (
    <div className="space-y-24 pb-32 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 animate-in fade-in slide-in-from-left duration-700 relative z-10">
            <div className="inline-flex items-center gap-3 px-4 py-2 bg-brand-500/10 border border-brand-500/20 rounded-2xl">
              ...
              <span className="text-[10px] font-black text-brand-400 uppercase tracking-[0.2em]">Online: TizzyBeatz Studio</span>
            </div>
            ...
            <p className="text-xl text-slate-400 max-w-lg leading-relaxed font-medium">
              High-quality beats for your next hit song. Find the perfect sound to elevate your music.
            </p>
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              >
                Browse Beats <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform" />
              </button>
              <button
              ...
              >
                <MessageCircle size={20} className="text-brand-400 group-hover:rotate-12 transition-transform" /> Work With Me
              </button>
            </div>
            ...
            <div className="pt-8 flex items-center gap-8 border-t border-white/5">
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Beats Available</p>
              </div>
          ...
          <div className="relative hidden lg:block animate-in fade-in zoom-in duration-1000">
            ...
            <div className={`relative aspect-square rounded-[60px] overflow-hidden border border-white/10 glass p-4 shadow-3xl group ${isPlaying ? 'animate-beat' : 'animate-float'}`}>
              ...
              <div className="absolute inset-x-8 bottom-8 p-6 glass rounded-3xl border border-white/10 flex items-center justify-between backdrop-blur-md">
                <div>
                  <p className="text-[10px] font-black text-brand-400 uppercase tracking-widest">Newest Release</p>
        ...
        <div className="flex flex-col md:flex-row items-end justify-between gap-10">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <h3 className="text-5xl font-black text-white tracking-tighter">BEAT STORE</h3>
              <span className="px-3 py-1 bg-brand-500/20 text-brand-400 rounded-lg text-[10px] font-black border border-brand-500/20">AVAILABLE NOW</span>
            </div>
            ...
            <div key={product.id} id={`product-${product.id}`} className="neo-brutalism-card group flex flex-col scroll-mt-24">
              <div className="relative aspect-[3/4] rounded-[56px] overflow-hidden mb-8 bg-slate-900 border border-white/10 shadow-3xl">
                ...
                <div className="absolute inset-0 pb-20 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center backdrop-blur-[4px] bg-brand-900/10 z-20">
                  ...
                  <p className="mt-4 text-[10px] font-black text-white uppercase tracking-[0.3em] opacity-80">Play</p>
                </div>

                {/* Tactical Badges */}
                <div className="absolute top-6 left-6 flex flex-col gap-3 z-30">
                  {product.isFree && (
                    <span className="px-4 py-1.5 bg-pink-600 text-white text-[10px] font-black uppercase tracking-[0.25em] rounded-xl shadow-xl animate-pulse">FREE BEAT</span>
                  )}
                  {product.bpm && (
                ...
                <div className="absolute bottom-8 left-8 right-8 z-10 pointer-events-none">
                  ...
                  <div className="flex items-center gap-3 mt-3 opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100">
                    ...
                    <span className="text-[10px] font-black text-brand-400 uppercase tracking-widest">HIGH QUALITY FILES</span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 space-y-6">
                ...
                <div className="flex gap-4">
                  {product.isFree ? (
                    ...
                    >
                      <Download size={20} className="group-hover/btn:scale-110 group-hover/btn:-translate-y-1 transition-transform" /> Download Now
                    </button>
                  ) : (
                    <button
                    ...
                    >
                      <ShoppingCart size={20} className="group-hover/btn:scale-110 group-hover/btn:rotate-12 transition-transform" /> Add to Cart
                    </button>
                  )}
                </div>
              </div>
      ...
      <section className="relative glass rounded-[48px] p-12 border border-white/5 overflow-hidden group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center relative z-10">
          ...
          <div className="md:col-span-8 space-y-6 text-center md:text-left">
            <div className="inline-block px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest text-brand-400">About Me</div>
            <h2 className="text-6xl font-black text-white tracking-tighter leading-[0.9]">"CREATING THE PERFECT <br />SOUND FOR YOUR NEXT TRACK."</h2>
            <p className="text-slate-400 leading-relaxed text-xl max-w-2xl font-medium">
              I'm <span className="text-white font-black underline decoration-accent-500 decoration-4 underline-offset-8">TizzyBeatz</span>. 2 years producing beats that stand out. Every track is made with one goal: helping artists make their best music.
            </p>
            <div className="flex flex-wrap justify-center md:justify-start gap-12 pt-8">
              {[
                { label: '50k+', sub: 'Streams & Downloads' },
      ...
      <section className="relative px-8 py-28 lg:p-36 rounded-[100px] bg-slate-900 border border-white/5 overflow-hidden shadow-3xl group">
        ...
        <div className="relative z-10 max-w-5xl space-y-12">
          <div className="inline-flex items-center gap-4 px-6 py-3 bg-brand-600/20 rounded-2xl border border-brand-500/20 backdrop-blur-xl">
            ...
            <span className="text-xs font-black text-white uppercase tracking-[0.4em]">CUSTOM BEATS & COLLABORATION</span>
          </div>
          <h2 className="text-7xl lg:text-9xl font-black text-white leading-[0.8] tracking-tighter">LET'S MAKE A <br /><span className="text-brand-500 italic">HIT</span> TOGETHER.</h2>
          <p className="text-2xl text-slate-400 font-medium leading-relaxed max-w-3xl">
            Looking for a custom beat, mixing, or exclusive production? I take on a limited number of projects each month. Let's work.
          </p>
          <button
            onClick={() => setShowCollabModal(true)}
          ...
          >
            Contact Me <ArrowRight size={32} className="inline-block ml-4 group-hover/btn:translate-x-3 transition-transform" />
          </button>
        </div>
      ...
      <footer className="pt-32 border-t border-white/5 flex flex-col lg:flex-row items-start justify-between gap-24 px-8 relative overflow-hidden">
        ...
        <div className="space-y-8 max-w-lg relative z-10">
          ...
          <p className="text-xl text-slate-500 font-medium leading-relaxed">
            High-quality beats for your next project. Professional production for artists who want to sound their best.
          </p>
          <div className="flex gap-4">
            {[1, 2, 3].map(i => (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-20 lg:gap-32 relative z-10">
          <div className="space-y-8">
            <p className="text-[12px] font-black text-white uppercase tracking-[0.3em] border-b border-brand-500/50 pb-2 inline-block">SOCIALS</p>
          ...
          <div className="space-y-8">
            <p className="text-[12px] font-black text-white uppercase tracking-[0.3em] border-b border-brand-500/50 pb-2 inline-block">INFO</p>
            <ul className="space-y-6 text-sm font-bold text-slate-500">
              ...
              <li className="hover:text-brand-400 cursor-pointer transition">Privacy Policy</li>
              <li className="hover:text-brand-400 cursor-pointer transition">Contact Support</li>
            </ul>
          </div>
          ...
          <div className="bg-slate-900 w-full max-w-2xl rounded-[40px] sm:rounded-[60px] border border-white/10 p-8 sm:p-12 shadow-[0_0_120px_rgba(99,102,241,0.25)] relative overflow-hidden animate-in fade-in zoom-in duration-500 my-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            {isSuccess ? (
              <div className="py-24 flex flex-col items-center text-center space-y-10">
                ...
                <div className="space-y-4">
                  <h3 className="text-3xl sm:text-5xl font-black text-white uppercase tracking-tighter">REQUEST SENT</h3>
                  <p className="text-slate-400 text-lg sm:text-xl font-medium max-w-sm mx-auto leading-relaxed">Thanks for reaching out! I'll get back to you within 24 hours.</p>
                </div>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between mb-8 sm:mb-12">
                  <div className="flex items-center gap-4 sm:gap-6">
                    ...
                    <div>
                      <h2 className="text-2xl sm:text-4xl font-black text-white uppercase tracking-tight">REQUEST COLLABORATION</h2>
                      <p className="text-[9px] sm:text-[11px] text-slate-500 font-black uppercase tracking-[0.3em] mt-1">Let's work together on your next track</p>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmitCollab} className="space-y-6 sm:space-y-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                    <div className="space-y-3">
                      <label className="block text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Your Name</label>
                    ...
                    <div className="space-y-3">
                      <label className="block text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Project Type</label>
                  ...
                  <div className="space-y-3">
                    <label className="block text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Your Email Address</label>
                    <input required name="email" type="email" className="w-full bg-slate-950/80 border border-white/5 rounded-[24px] sm:rounded-[32px] px-6 sm:px-8 py-4 sm:py-6 text-slate-100 font-bold focus:ring-4 focus:ring-brand-500/10 outline-none transition-all placeholder:text-slate-700 text-base sm:text-lg" placeholder="artist@email.com" />
                  </div>
                  <div className="space-y-3">
                    <label className="block text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Project Details</label>
                  ...
                  <div className="flex gap-4 pt-4 sm:pt-6">
                    <button type="submit" className="w-full py-6 sm:py-8 bg-brand-600 hover:bg-brand-500 text-white font-black rounded-[32px] sm:rounded-[40px] transition-all shadow-3xl shadow-brand-600/50 uppercase tracking-[0.3em] text-base sm:text-lg active:scale-95 group">
                      Send Request <ArrowRight size={24} className="inline-block ml-3 group-hover:translate-x-2 transition-transform" />
                    </button>
                  </div>
                </form>
              </>
      ...
      <LicenseSelectionModal
        isOpen={!!activeLicenseProduct}
        product={activeLicenseProduct}
        onClose={() => setActiveLicenseProduct(null)}
